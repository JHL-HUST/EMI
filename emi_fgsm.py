# coding=utf-8
"""Implementation of EMI-FGSM attack."""
...
tf.flags.DEFINE_integer('sampling_interval', 7, 'interval size for lookahead')
s
tf.flags.DEFINE_integer('sampling_number', 11, 'number of samples for lookahead')
...
def graph(x, y, i, x_max, x_min, grad, lgrad):
    ...
    sampling_number = FLAGS.sampling_number
    sampling_interval = FLAGS.sampling_interval
    factors = np.linspace(-sampling_interval, sampling_interval, num=sampling_number)
    x_lookaheads = [x+factor*alpha*lgrad for factor in factors]
    x_input = tf.concat(x_lookaheads, axis=0)
    ...
    one_hot = tf.concat([tf.one_hot(y, num_classes)] * sampling_number, axis=0)

    cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits_v3)
    new_grad = tf.reduce_sum(tf.split(tf.gradients(cross_entropy, x_input)[0], sampling_number), axis=0)

    new_grad = new_grad / tf.reduce_mean(tf.abs(new_grad), [1, 2, 3], keep_dims=True)

    noise = momentum * grad + new_grad

    x = x + alpha * tf.sign(noise)
    x = tf.clip_by_value(x, x_min, x_max)
    i = tf.add(i, 1)

    return x, y, i, x_max, x_min, noise, new_grad


def stop(x, y, i, x_max, x_min, grad, lgrad):
    num_iter = FLAGS.num_iter
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        x_adv, _, _, _, _, _, _ = tf.while_loop(stop, graph, [x_input, y, i, x_max, x_min, grad, tf.zeros(shape=batch_shape)])
