...
def graph(x, y, i, x_max, x_min, grad, lgrad):
    ...
    new_gradient = tf.gradients(cross_entropy, x)[0]

    new_gradient = new_gradient / tf.reduce_mean(tf.abs(new_gradient), [1, 2, 3], keep_dims=True)

    noise = momentum * grad + new_gradient

    x = x + alpha * tf.sign(noise)
    x = tf.clip_by_value(x, x_min, x_max)
    i = tf.add(i, 1)

    return x, y, i, x_max, x_min, noise, new_gradient
